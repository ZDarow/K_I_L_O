...
### Python-инструментарий (uv)

Проект использует [uv](https://docs.astral.sh/uv/) как единый менеджер Python-пакетов
(замена pip/venv/pipx).
...
# Обновление lock-файла и зависимостей
uv lock --upgrade
...
# yamllint — проверка YAML (через uv)
uv sync
make lint-yaml
