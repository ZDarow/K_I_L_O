...
- Длина заголовка ≤ 72 символа, тело также форматируется по 72 символа.
- Пример:  
  `feat: добавить поддержку BLE-нотификаций`

### Ветки

- Названия на **английском**, kebab-case.
- Рекомендуемая модель — **упрощённый Git Flow**:
  - `main` – стабильный релизный код
  - `develop` – основная ветка разработки
  - `feature/<what>` – новая функциональность
  - `fix/<what>` – исправление бага  - `release/<version>` – подготовка релиза
  - `hotfix/<what>` – срочное исправление в `main`

Примеры:  
`feature/ble-notifications`, `fix/volume-range`, `release/v2.1.0`
